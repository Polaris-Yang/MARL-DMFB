...
class CRNN(nn.Module):
    def __init__(self, args):
      ...
      # self.bn2 = nn.BatchNorm2d(32)
        self.mlp1 = nn.Linear(args.n_agents+args.n_actions, 10)
        self.rnn = nn.GRUCell(5*12*32+10, args.rnn_hidden_dim)
        self.fc1 = nn.Linear(args.rnn_hidden_dim, args.n_actions)

    def forward(self, inputs, hidden_state):
        pixel, vec = torch.split(
            inputs, [60*30*3, self.args.n_agents+self.args.n_actions], dim=1)
        # pixel = pixel.cuda()
        # vec = vec.cuda()
        pixel = pixel.reshape((-1, 3, 30, 60))
        pixel = f.relu(self.conv1(pixel))
        pixel = f.relu(self.conv2(pixel))
        pixel = pixel.reshape((-1, 5*12*32))  # (batch,800)
