...
class Runner:
    def __init__(self, env, args):
        ...
        self.success_rate = []
        # 用来保存plt和pkl
        self.save_path = self.args.result_dir + '/' + args.alg +'/' + '{}by{}-{}d'.format(self.args.width,self.args.length,self.args.drop_num)
    ...
    def run(self, num):
        ...
        while time_steps < self.args.n_steps:
            ...
            if time_steps // self.args.evaluate_cycle > evaluate_steps:
                average_episode_reward, average_episode_steps, success_rate = self.evaluate()
            ...
            for episode_idx in range(self.args.n_episodes):
                episode, _, steps, _ = self.rolloutWorker.generate_episode(episode_idx)
        ...
        self.agents.policy.save_final_model()
    ...
    def evaluate(self):
        ...
        for epoch in range(self.args.evaluate_epoch):
            ...
            _, episode_reward, total_step, success = self.rolloutWorker.generate_episode(epoch,evaluate=True)
            episode_rewards += episode_reward
            episode_steps += total_step
            total_success += success
        return episode_rewards / self.args.evaluate_epoch, episode_steps / self.args.evaluate_epoch, total_success / self.args.evaluate_epoch
    ...
    def plt(self, num):
        ...
        y_name = ['Rewards', '$T_{latest}$', 'success\_rate']
        x_name = 'step*' + str(self.args.evaluate_cycle)
        data = [self.episode_rewards, self.episode_steps,self.success_rate]
        for i in range(3):
            plt.subplot(3, 1, i + 1)
    ...
    def train_data_save(self, num):
        prefix = '/{}'.format(self.args.alg) + '_env({},{},{})'.format(
            self.args.width, self.args.length, self.args.drop_num)
